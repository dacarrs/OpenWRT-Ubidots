...
function internet_up()
  local f = io.popen("ping -c1 8.8.8.8 | grep ttl")
  local l = f:read("*a")
  f:close()
  if l == '' then
...
function post_value(deviceID, data)
   ...
   -----------------
   local f = io.popen(string.format('ping -c 1 -W 4 8.8.8.8 | grep ttl', var_ip))
   local l = f:read("*a")
   f:close()
   local rtime = '-1'
   if l ~= '' then
  ...
  -----------
  --local value = "\'{\"value\":".. data .."}\'"
  local value = "\'{\"value\":".. rtime .."}\'"
  
  print("curl -i --header ".. Head1 .. " --header " .. Head2 .. " --header " .. api_token .. " -X POST -d " .. value .. dataURL) 
...
function print_usage() 
  ...
  print("Example: Post ping response time:")
  print("ubitos -post your_api_key_here your_deviceID_here\n")
  return
end
